# Fantomaas Scraper Bot — Demo Specification
...
Tool descriptions below are verbatim what goes in the OpenAPI schema. Claude reads these to route correctly.
...
> Filters the cleaned Egyptian projects table by any combination of governorate name, category, expected completion year range, and project cost range in EGP. Returns matching projects with all clean fields. Use for any question about specific projects, locations, categories, or completion timelines. Do NOT use for CBE economic indicators.

### `estimate_steel_total(filters)`
> Aggregates steel estimates across a filtered set of projects. Returns total tons (typical), low/high band, count of projects with insufficient data excluded, and the top 3 contributors by tonnage. Use for "how much steel" questions across multiple projects. For single-project estimates, use query_projects and read the tons columns directly.
...
1. SQLite file `scraperbot.db` with all tables from Section 4. Each table has 3–10 rows of plausible hand-typed dummy data. `cleaning_log` has its eight rows. `steel_ratios` has ~10 rows (replaced later with full GPT table).

2. FastAPI service runs on localhost. All nine tools from Section 5 implemented. Each reads from SQLite and returns structured JSON. Swagger UI loads cleanly at `/docs`.

3. Test script that calls each of the nine tools via HTTP and confirms a sensible response. All nine pass = FastAPI done for Step 1.

4. Standalone Streamlit chat UI exists. Uses `st.chat_input`, holds conversation state, has Claude wired with the system prompt from Appendix A, tool definitions auto-loaded from FastAPI's OpenAPI schema. Welcome message renders. Four suggested-prompt buttons render and submit when clicked.

5. All seven canned demo prompts produce a non-error response from the dummy-data version. Some will be unimpressive — that's fine. The point is structural correctness.
